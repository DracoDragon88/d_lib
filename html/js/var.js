    ...
    if (var_started && valid_keys.includes(key_pressed)) {
        switch (key_pressed) {
            case 'Escape':
                ...
                game_playing = false;
                destroy()
                $.post(`https://${GetParentResourceName()}/game-callback`, JSON.stringify({ 'success': false, "fails": numbers-order }));
    ...
    if (parseInt(ev.target.dataset.number, 10) === order) {
        ...
        if(order > numbers){
            ...
            setTimeout(function() { 
                ...
                destroy()
                $.post(`https://${GetParentResourceName()}/game-callback`, JSON.stringify({ 'success': true, "fails": 0 }));
            }, 2000);
        }
    } else {
        //destroy()
