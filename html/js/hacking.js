...
const getRandomIntH = (min, max) => Math.floor(Math.random() * (max - min + 1)) + min;
...
const tryFinish = () => {
    if (solved.every((val) => val)) {
        ...
        setTimeout(() => {
            ...
            $.post(`https://${GetParentResourceName()}/game-callback`, JSON.stringify({ 'success': true, 'fails': mistakes }));
            resetGameState();
        }, 2000);
    }
};
...
const seedTable = (timestamp) => {
    ...
    if (now >= endTime) {
        ...
        setTimeout(() => {
            ...
            $.post(`https://${GetParentResourceName()}/game-callback`, JSON.stringify({ 'success': false, 'fails': mistakes }));
            resetGameState();
        }, 2000);
        return;
    }
    ...
    if (timestamp - lastUpdateTime > 200) {
        for (let i = 0; i < NUM_ROWS; i++) {
            for (let j = 0; j < NUM_COLS; j++) {
                ...
                if (cell) {
                    gameTable[i][j] = HEX_VALUES[getRandomIntH(0, 15)];
...
const generateSolutions = (s) => {
    solutionPos[0] = [getRandomIntH(1, 9), getRandomIntH(1, 18 - s)];
    let goodsolution = false;
    while (!goodsolution) {
        solutionPos[1] = [getRandomIntH(1, 9), getRandomIntH(1, 18 - s)];
    ...
    for (let i = 0; i < s; i++) {
        solutions[0] = solutions[0] + HEX_VALUES[getRandomIntH(0, 15)];
        solutions[1] = solutions[1] + HEX_VALUES[getRandomIntH(0, 15)];
    }
    userSolPos = [
        [1, 1],
        [1, 19 - s],
...
const startGameHacking = (solutionsize, timeout) => {
    initializeHackingGameTable();
    for (let i = 0; i < NUM_ROWS; i++) {
        for (let j = 0; j < NUM_COLS; j++) {
            lastReseed[i][j] = 0;
        }
...
const initializeHackingGameTable = () => {
...
window.addEventListener("message", (event) => {
    ...
    if (data.action === "startHack") {
        if (!gameStarted) {
            ...
            startGameHacking(data.solutionsize, data.timeout);
        }
    }
});
