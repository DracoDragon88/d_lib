...
---@field job fun(task: DTask, date: osdate)
---@field isActive boolean
---@field id number
---@field debug? boolean

---@class DTask : DTaskProperties
---@field expression string
---@field private scheduleTask fun(self: DTask): boolean?
local DTask = {}
DTask.__index = DTask
...
---@return number
function DTask:getAbsoluteNextTime()
    local minute = getTimeUnit(self.minute, 'min')
    local hour = getTimeUnit(self.hour, 'hour')
    local day = getTimeUnit(self.day, 'day')
    local month = getTimeUnit(self.month, 'month')
...
function DTask:getTimeAsString(timestamp)
...
---@param job fun(task: DTask, date: osdate)
...
function dlib.cron.new(expression, job, options)
    ...
    ---@type DTask
    local task = setmetatable(options or {}, DTask)
...
---Converts seconds to cron expression
---@param time number seconds
---@return string expression `* * * * *`
function dlib.cron.timeconvert(time)
