...
---@param round? boolean
---@return number? ...
local function math_toscalars(input, min, max, round)
    local arr = {}
    local i = 0
...
local function math_tovector(input, min, max, round)
...
---@param input vector3
---@return vector3
local function math_normaltorotation(input)
    local inputType = type(input)
...
---@param input string | table
---@return vector4
local function math_torgba(input)
...
---@return integer
---@return integer
local function math_hextorgb(input)
...
---@param upper? boolean
---@return string
local function math_tohex(n, upper)
...
---@return string
local function math_groupdigits(number, seperator) -- credit http://richard.warburton.it
...
---@param upper number
---@return number
local function math_clamp(val, lower, upper) -- credit https://love2d.org/forums/viewtopic.php?t=1856
...
---@return fun(): T, number
function math_lerp(start, finish, duration)
    ...
    local interpFn = typeStart == 'table' and interpolateTable or math_interp
    local step
...
---@return number
local function math_round(value, places)
...
if not math.toscalars then math.toscalars = math_toscalars end
if not math.tovector then math.tovector = math_tovector end
if not math.normaltorotation then math.normaltorotation = math_normaltorotation end
if not math.torgba then math.torgba = math_torgba end
if not math.hextorgb then math.hextorgb = math_hextorgb end
if not math.tohex then math.tohex = math_tohex end
if not math.groupdigits then math_groupdigits = math_groupdigits end
if not math.clamp then math.clamp = math_clamp end
if not math.interp then math.interp = math_interp end
