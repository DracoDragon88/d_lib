...
---@param ... any
function dlib.addCommand(commandName, properties, cb, ...)
    ...
    if properties then
        if ... or table.type(properties) ~= 'hash' then
            ...
            warn(("command '%s' is using deprecated syntax for dlib.addCommand\nupdate the command or use dlib.__addCommand to ignore this warning\n> source ^0(^5%s^0:%d)"):format(_commandName, info.short_src, info.currentline))
            ---@diagnostic disable-next-line: deprecated
            return dlib.__addCommand(commandName, properties, cb, ...)
        end
