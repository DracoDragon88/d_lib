...
function dlib.raycast.fromCoords(coords, destination, flags, ignore)
...
function dlib.raycast.fromCamera(flags, ignore, distance)
    local coords = GetFinalRenderedCamCoord()
    local destination = coords + getForwardVector() * (distance or 10)

    return dlib.raycast.fromCoords(GetFinalRenderedCamCoord(), destination, flags, ignore)
end

---@deprecated
dlib.raycast.cam = dlib.raycast.fromCamera
