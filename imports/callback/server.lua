...
local cbEvent = '__dlib_cb_%s'
local callbackTimeout = GetConvarInt('dlib:callbackTimeout', 300000)
...
function dlib.callback.await(event, playerId, ...)
...
---@diagnostic disable-next-line: duplicate-set-field
function dlib.callback.register(name, cb)
    RegisterNetEvent(cbEvent:format(name), function(resource, key, ...)
        TriggerClientEvent(cbEvent:format(resource), source, key, callbackResponse(pcall(cb, source, ...)))
    end)
end

return dlib.callback
