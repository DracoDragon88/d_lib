...
function dlib.__addCommand(group, name, callback, parameters, help)
    if not group then group = 'builtin.everyone' end

    if type(name) == 'table' then
        for i = 1, #name do
            ---@diagnostic disable-next-line: deprecated
            dlib.__addCommand(group, name[i], callback, parameters, help)
        end
    else
        ...
        if type(group) == 'table' then
            for _, v in ipairs(group) do
                if not IsPrincipalAceAllowed(v, name) then dlib.addAce(v, name) end
            end
        else
            if not IsPrincipalAceAllowed(group, name) then dlib.addAce(group, name) end
        end
    end
end

---@diagnostic disable-next-line: deprecated
return dlib.__addCommand
