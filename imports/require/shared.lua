...
local function getModuleInfo(modName)
    ...
    while true do
        ...
        if resource and not src:find('^@@d_lib/imports/require') then
...
function dlib.load(filePath, env)
...
function dlib.loadJson(filePath)
    if type(filePath) ~= 'string' then
        error(("file path must be a string (received '%s')"):format(filePath), 2)
    end
