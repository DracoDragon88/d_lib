...
function dlib.addAce(principal, ace, allow)
...
function dlib.removeAce(principal, ace, allow)
	if type(principal) == 'number' then
		principal = 'player.'..principal
	end
...
function dlib.addPrincipal(child, parent)
...
function dlib.removePrincipal(child, parent)
	if type(child) == 'number' then
		child = 'player.'..child
	end
...
dlib.callback.register('d_lib:checkPlayerAce', function(source, command)
