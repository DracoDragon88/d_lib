...
RegisterNetEvent('d_lib:setVehicleProperties', function(netid, data)
    local timeout = 100
    ...
    if timeout > 0 then
        dlib.setVehicleProperties(NetToVeh(netid), data)
    end
end)

AddStateBagChangeHandler('d_lib:setVehicleProperties', '', function(bagName, _, value)
    if not value or not GetEntityFromStateBagName then return end

    while NetworkIsInTutorialSession() do Wait(0) end

    local entityExists, entity = pcall(dlib.waitFor, function()
    ...
    dlib.setVehicleProperties(entity, value)
    Wait(200)
    ...
    if NetworkGetEntityOwner(entity) == cache.playerId then
        dlib.setVehicleProperties(entity, value)
        Entity(entity).state:set('d_lib:setVehicleProperties', nil, true)
    end
end)
...
function dlib.getVehicleProperties(vehicle)
...
function dlib.setVehicleProperties(vehicle, props, fixVehicle)
